...
public class Chat extends HideableUIElement {
	private final float[] uiColor = new float[]{0.4f, 0.4f, 0.4f, 0.4f};
	private final float chatHeight = 240f;
	private final float chatWidth = 0.65f;
	private GLString[] entries = new GLString[(int) (chatHeight / 20f)];
	private int width;
	private int vbo = -1;
	private String renderText;
	
	public Chat(int width) {
		renderText = " > ";
		for (int i = 0; i < entries.length; i++) {
			entries[i] = GLStringUtils.getString("");
		}
		this.width = width;
		this.hide();
	}
	...
	public void render() {
		...
		buf.put(uiColor);
		buf.put(new float[]{8f + (width * chatWidth), 8f});
		buf.put(uiColor);
		buf.put(new float[]{8f + (width * chatWidth), 20f + 8f});
		buf.put(uiColor);
		buf.put(new float[]{8f, 20f + 8f});
		buf.put(uiColor);
		buf.put(new float[]{8f, 20f + 8f + 4f});
		buf.put(uiColor);
		buf.put(new float[]{8f + (width * chatWidth), 20f + 8f + 4f});
		buf.put(uiColor);
		buf.put(new float[]{8f + (width * chatWidth), 20f + 8f + 4f + chatHeight});
		buf.put(uiColor);
		buf.put(new float[]{8f, 20f + 8f + 4f + chatHeight});
		buf.put(uiColor);
		...
		GLStringUtils.getString(renderText).render(8f, 27f);
		for (int i = 0; i < entries.length; i++) {
			entries[i].render(8f, 8f + 4f + 20f + (20f * (entries.length - i)));
		}
	}
	...
	private void vboBufferData(int id) {
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, vbo);
		GL15.glBufferData(GL15.GL_ARRAY_BUFFER, 2 * 4 * 6 * Constants.SIZEOF_FLOAT, GL15.GL_DYNAMIC_DRAW);
	}
}
